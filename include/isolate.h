...
class Isolate {
 public:
  virtual vector<tuple<double, double>> isoRoot(Poly& p) = 0;
  ...
  static Poly addToP(Poly& p, double h) {
    int N = p.size();
    vector<double> tmp(N, 0.0);
  ...
  static Poly timeToP(Poly& p, double h) {
  ...
  static int signChangeNum(Poly& tmp, double h) {
    Poly p = addToP(tmp, h);
    int ret = 0;
    bool sign = p[0] > 0;
